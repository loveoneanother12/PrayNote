    ...
    <>
      <section className="inline-composer">
        ...
        <form onSubmit={submit}>
          ...
          <textarea id="group-prayer-content" name="content" aria-describedby="group-prayer-guidance" maxLength={2000} required value={content} onChange={(event) => setContent(event.target.value)} placeholder="함께 기도받고 싶은 내용을 적어주세요." />
          <p className="composer-guidance" id="group-prayer-guidance">이 그룹의 멤버에게 공유돼요. · {content.length.toLocaleString()} / 2,000</p>
          <div><span role={error ? "alert" : undefined}>{error || "작은 기도도 함께 나눠요."}</span><button className={`primary-button ${pending ? "button-pending" : ""}`} type="submit" disabled={pending || !content.trim()}>{pending ? <><LoaderCircle className="button-spinner" size={15} />등록 중…</> : <><Plus size={17} />기도제목 등록</>}</button></div>
        </form>
      </section>
