...
export function SharePrayerModal({
  ...
  sharedGroups?: ShareGroup[];
  groups: ShareGroup[];
  returnTo: string;
  onClose?: () => void;
  onShared?: (groupIds: string[]) => void;
}) {
  const router = useRouter();
  const [selected, setSelected] = useState<string[]>([]);
  const [pending, setPending] = useState(false);
  const [error, setError] = useState("");
  const existingGroups = sharedGroups ?? (currentGroup ? [currentGroup] : []);
  const existingGroupIds = new Set(existingGroups.map((group) => group.id));
  const otherGroups = groups.filter((group) => !existingGroupIds.has(group.id));
    ...
    <div className="modal-backdrop" role="presentation" onMouseDown={close}>
      <div className="composer-modal share-prayer-modal" role="dialog" aria-modal="true" aria-labelledby="share-prayer-title" onMouseDown={(event) => event.stopPropagation()}>
        <div className="composer-heading">
          <div><span className="overview-icon"><Share2 size={20} /></span><div><h2 id="share-prayer-title">다른 그룹에도 공유할까요?</h2><p>이 기도제목을 여러 공동체에 함께 나눌 수 있어요.</p></div></div>
          <button type="button" onClick={close} aria-label="닫기">×</button>
        </div>
        <form onSubmit={share}>
          <div className="share-group-list">
            {existingGroups.map((group) => (
              <label className="share-group-option selected disabled" key={group.id}>
                <input type="checkbox" checked disabled readOnly />
                <span><Users size={15} /><span><strong>{group.name}</strong><small>이미 공유 중인 그룹</small></span></span>
                <Check size={16} />
              </label>
            ))}
