-- Persist a per-user group order shared by the groups page and dashboard.

alter table public.group_memberships
  add column if not exists display_order integer;

create index if not exists group_memberships_user_display_order_idx
  on public.group_memberships(user_id, display_order, created_at)
  where status = 'active';

create or replace function public.reorder_my_groups(ordered_group_ids uuid[])
returns void
language plpgsql
security definer
...
begin
  ...
  from unnest(coalesce(ordered_group_ids, '{}'::uuid[])) as requested(group_id);

  if supplied_count <> distinct_count then
    raise exception 'Duplicate group IDs are not allowed';
  end if;
  ...
  if exists (
    ...
    where not exists (
      ...
      where membership.user_id = auth.uid()
        and membership.group_id = requested.group_id
        and membership.status = 'active'
    )
  ) then
    raise exception 'Invalid group order';
  end if;

  update public.group_memberships membership
  set display_order = ordering.position::integer, updated_at = now()
  from unnest(coalesce(ordered_group_ids, '{}'::uuid[])) with ordinality as ordering(group_id, position)
  where membership.user_id = auth.uid()
    and membership.group_id = ordering.group_id
    and membership.status = 'active';
end;
$$;
...
as $$
  select case when auth.uid() is null then '[]'::jsonb else coalesce((
    select jsonb_agg(
      jsonb_build_object('id', app_group.id, 'name', app_group.name)
      order by membership.display_order nulls last, membership.created_at, app_group.name
    )
...
as $$
  with active_groups as materialized (
    select membership.group_id, membership.role, membership.created_at, membership.display_order
    from public.group_memberships membership
    join public.groups app_group on app_group.id = membership.group_id and app_group.deleted_at is null
    where membership.user_id = auth.uid() and membership.status = 'active'
    ...
    where prayer.status = 'active'
      ...
      and prayer.hidden_at is null
      and (
        (prayer.author_id = auth.uid() and not exists (
          select 1 from public.prayer_group_shares personal_share where personal_share.prayer_id = prayer.id
        ))
        or exists (
          select 1 from public.prayer_group_shares group_share
      ...
      'groups', coalesce((
        select jsonb_agg(jsonb_build_object(
          ...
          'active_prayer_count', (
            ...
            from public.prayer_group_shares share
            join public.prayer_requests prayer on prayer.id = share.prayer_id
            where share.group_id = app_group.id and prayer.status = 'active'
              and prayer.deleted_at is null and prayer.hidden_at is null
          ),
          'unread_count', 0
        ) order by active_group.display_order nulls last, active_group.created_at, app_group.name)
  ...
  select case when auth.uid() is null then null else jsonb_build_object(
    ...
    'group_prayers', coalesce((
      ...
      cross join lateral public.get_prayer_summaries_fast(
        target_group_ids => array[active_group.group_id], target_status => 'active', result_limit => 3
      ) prayer
    ...
    'personal_prayers', coalesce((
      ...
      from public.get_prayer_summaries_fast(
        target_author_id => auth.uid(), target_status => 'active', result_limit => 3, target_personal_only => true
      ) prayer
      left join public.profiles author on author.id = prayer.author_id
    ), '[]'::jsonb),
...
revoke all on function public.reorder_my_groups(uuid[]) from public, anon;
grant execute on function public.reorder_my_groups(uuid[]) to authenticated;
