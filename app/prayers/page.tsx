...
export default async function MyPrayersPage({ searchParams }: MyPrayersPageProps) {
  ...
  const [{ data: userData }, queryParams] = await Promise.all([supabase.auth.getUser(), searchParams]);
  ...
  const [{ data: profile }, prayers] = await Promise.all([
    ...
    getPrayerSummaries(supabase, userData.user.id, { authorId: userData.user.id }),
  ]);
      ...
      <main className="main-content subpage-main">
        ...
        <div className="content-wrap detail-content">
          ...
          {(view === "resolved" || view === "all") && <section className="prayer-record-section resolved-section">
            ...
            <div className="record-grid">
              {resolvedPrayers.map((prayer) => <PrayerRecordCard key={prayer.id} prayer={prayer} currentUserId={userData.user.id} returnTo="/prayers" showGroup />)}
